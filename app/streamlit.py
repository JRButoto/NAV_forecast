...
loaded_wekeza= pickle.load(open('models/Wekeza Maisha fund predictor.pkl','rb'))
...
st.sidebar.markdown('<center><h1 class="sidebar-header" style="color: #fff;"> UTT AMIS  NAV FORECASTING</h1></center>', unsafe_allow_html=True)
# Rest of your sidebar content...

page_selection = st.sidebar.radio('Select a Page', ['Home','Dataset Viewer', 'NAV Trend Yearly','Visualization', 'Predictions','Investment Calculator'])
...
if page_selection == 'Home':
    st.markdown('<div align="center"><h2 style="color: #A28089;>' + pages[page_selection] + '</h2></div>', unsafe_allow_html=True)
     
    st.markdown('<div align="center"><h2 style="color: #A28089;">UTT AMIS FUNDS</h2></div>', unsafe_allow_html=True)
    ...
    st.markdown('<hr style="height:5px; border:none; color:#A28089; background-color:#A28089;">', unsafe_allow_html=True)
...
# Add the CSS styles
    ...
    with col8:
        ...
        st.markdown ('<center><h6 class="white-h6"> DATASET VIEWER  </h6></center>',  unsafe_allow_html=True)

    with col9:
        st.image('images/NAV.png', width=40)
        st.markdown ('<center><h6 class="white-h6"> DATA ANALYTICS </h6></center>',  unsafe_allow_html=True)

    with col10:
        st.image('images/statistics.png', width=40)
        st.markdown ('<center><h6 class="white-h6"> DATA VISUALIZATION </h6></center>',  unsafe_allow_html=True)

    with col11:
        st.image('images/predictive-chart.png', width=40)
        st.markdown ('<center><h6 class="white-h6"> FORECASTING</h6></center>',  unsafe_allow_html=True)

    st.markdown('---')
    st.markdown ('<center> 2023 ©️ dLab Tanzania Group 2 ™️ </center>',  unsafe_allow_html=True)


elif page_selection == 'Dataset Viewer':
    st.markdown('<div align="center"><h2>' + pages[page_selection] + '</h2></div>', unsafe_allow_html=True)
    year_to_display = st.sidebar.selectbox('Choose a Year', data['Date_Valued'].str[-4:].unique())
    ...
    scheme_name_to_display = st.sidebar.selectbox('Choose a Scheme', data['Scheme_Name'].unique())

    # Filter the data based on the selected year and scheme_Name
    filtered_data = data[(data['Date_Valued'].str[-4:] == year_to_display) & (data['Scheme_Name'] == scheme_name_to_display)]
    
    # Create a centered trend plot with increased size
    if not filtered_data.empty:
        fig = px.line(filtered_data, x='Date_Valued', y='Nav_Per_Unit', color='Scheme_Name', title=f'NAV for {year_to_display} - {scheme_name_to_display}')
        fig.update_layout(
            width=1000,  
            height=600,
    ...
    st.markdown('<div align="center"><h2 style="color: #A28089;>' + pages[page_selection] + '</h2></div>', unsafe_allow_html=True)
    st.markdown('<hr style="height:5px; border:none; color:#A28089; background-color:#A28089;">', unsafe_allow_html=True)
    st.markdown('<div align="center"><h2 style="color: #A28089;">Avarage NAV For Each Fund</h2></div>', unsafe_allow_html=True)
    year_to_display = st.sidebar.selectbox('Choose a Year (Bar Plot)', data['Date_Valued'].str[-4:].unique())
    filtered_data = data[data['Date_Valued'].str[-4:] == year_to_display]    
    # Creating a subplot with two vertical bar charts (NAV per unit and Total NAV)
    fig = go.Figure()

    # Bar chart for NAV per unit
    scheme_nav_per_unit = filtered_data.groupby('Scheme_Name')['Nav_Per_Unit'].mean().reset_index()
    ...
    df_grouped = df2.groupby('Scheme Name')[['Sale Price/Unit', 'Repurchase Price/Unit']].mean()
    st.markdown('<div align="center"><h2 style="color: #A28089;">Avarage Fund Perfomance</h2></div>', unsafe_allow_html=True)

    # Add a slider to select the year
    selected_year = st.slider('Select a Year', min_value=df2['Year'].min(), max_value=df2['Year'].max(), value=df2['Year'].min())
    ...
    df_grouped = filtered_df.groupby('Scheme Name')[['Sale Price/Unit', 'Repurchase Price/Unit']].mean().reset_index()
    ...
    st.markdown('<div align="center"><h2 style="color: #A28089;">Pie Chart For Distribution of Funds</h2></div>', unsafe_allow_html=True)
    year_to_display = st.sidebar.selectbox('Choose a Year (Pie Chart)', data['Date_Valued'].str[-4:].unique())
    filtered_data = data[data['Date_Valued'].str[-4:] == year_to_display]
    st.write(f'Pie Chart for Funds in {year_to_display}')
    scheme_nav_totals = filtered_data.groupby('Scheme_Name')['Net_Asset_Value'].sum().reset_index()
    ...
    st.markdown('<div align="center"><h2 style="color: #A28089;">Corelation Heatmap Matrix</h2></div>', unsafe_allow_html=True)

    correlation_matrix = filtered_df[['Sale Price/Unit', 'Repurchase Price/Unit', 'Units', 'NAV']].corr()
    ...
    # Streamlit app
    st.markdown('<div align="center"><h2 style="color: #A28089;">Sale Price Distribution Over Year</h2></div>', unsafe_allow_html=True)
    ...
    # Streamlit app
    st.markdown('<div align="center"><h2 style="color: #A28089;">Units Distribution Over Year</h2></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div align="left"><h5 style="color: #A28089;">Enter the details for prediction</h5></div>', unsafe_allow_html=True)
    ...
    if loaded_fund is not None:
        with st.spinner("predicting..."):
            specific_date = pd.DataFrame({'ds': [date]})  # Change the date as needed
            forecast = loaded_fund.predict(specific_date)
            specific_date_prediction = forecast.loc[0, 'yhat']
            st.success(f'Predicted Net Asset Value per unit:{specific_date_prediction:.2f} TSH')
elif page_selection == 'Predictions':
    st.markdown('<div align="center"><h2>' + pages[page_selection] + '</h2></div>', unsafe_allow_html=True)
    ...
    if loaded_fund is not None:
        with st.spinner("predicting..."):
            ...
            st.success(f'Total Amount due  {d2}  is :{specific_date_prediction*a:.2f} Tsh')
